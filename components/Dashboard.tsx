...
const Dashboard: React.FC<DashboardProps> = ({ entries, projects, userProfile, onViewChange }) => {
    ...
    const vatStats = useMemo(() => {
        ...
        const totalLifeEarnings = (entries || [])
            .filter(e => e.startTime >= VAT_OPENING_DATE)
            .reduce((acc, e) => acc + calculateEarnings(e), 0);
            
        return {
            total: totalLifeEarnings,
            dailyAvg: totalLifeEarnings / daysSinceOpening,
    ...
    const stats = useMemo(() => {
        ...
        // ANALISI FISCALE ANALITICA (Ing. Righini System)
    ...
    return (
        <div className="flex flex-col min-h-screen animate-fade-in max-w-6xl mx-auto space-y-8 pb-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
                    <div className="relative z-10">
                        <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-4">Liquidità Netta Reale (Cash)</p>
                ...
                {/* VAT PROGRESS NOTIFICATION - Corner Widget */}
                <div className="bg-indigo-600 p-8 rounded-[2.5rem] text-white shadow-xl relative overflow-hidden flex flex-col justify-between">
                    ...
                    <div>
                        ...
                        <p className="text-2xl font-black font-mono leading-none mb-2">{formatCurrency(vatStats.total)}</p>
                        <p className="text-[9px] font-bold uppercase tracking-widest text-indigo-200">Guadagno Life-time</p>
                    </div>
                    <div className="mt-6 p-4 bg-white/10 rounded-2xl border border-white/10">
                        <div className="flex justify-between items-center">
                            <span className="text-[9px] font-black uppercase text-white/60">Media Daily:</span>
                            <span className="text-sm font-black font-mono">{formatCurrency(vatStats.dailyAvg)}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
                    <div className="space-y-12">
                        ...
                        <div className="bg-indigo-50/50 p-8 rounded-[2.5rem] border border-indigo-100">
                            ...
                            <div className="flex justify-between items-center">
                                <span className="text-sm font-bold text-slate-600 italic">Disponibilità in cassa reale:</span>
                    ...
                    <div className="bg-slate-50 rounded-[2.5rem] p-10 space-y-8 border border-slate-100 shadow-inner">
                        ...
                        <div className="pt-8 border-t-2 border-slate-200 flex justify-between items-center">
                            <span className="text-sm font-black text-slate-900 uppercase tracking-[0.2em]">Totale Debito Fiscale Stimato:</span>
            ...
            <div className="pt-16 border-t border-slate-100 text-center">
                <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.6em] mb-4">FluxLedger ERP Professional • v1.7.0 • Studio Engineering Systems</p>
